...
describe('teal player test', () => {
  ...
  testName = test('play-pause-video', async() => {
    ...
    const pauseButton = await driver.wait(until.elementLocated(By.css('#pauseTealSvg')), global.ELEMENT_TIMEOUT);
    await driver.wait(until.elementIsVisible(pauseButton), global.ELEMENT_TIMEOUT);
    // Click the puase button
    await driver.findElement(By.css('#pauseTealSvg')).click();
    // Validate the pause button is hidden
    await driver.wait(until.elementLocated(By.css('.btn-index-1.tealplayer-vis-hidden')), global.ELEMENT_TIMEOUT);
    // Pause for a second
    await driver.sleep(1000);
    // Wait for the play button to display after video starts
    const playButton = await driver.wait(until.elementLocated(By.css('#playTealSvg')), global.ELEMENT_TIMEOUT);
    await driver.wait(until.elementIsVisible(playButton), global.ELEMENT_TIMEOUT);
    // Click the play button
    await driver.findElement(By.css('#playTealSvg')).click();
    // Validate the play button is hidden
    await driver.wait(until.elementLocated(By.css('.btn-index-0.tealplayer-vis-hidden')), global.ELEMENT_TIMEOUT);
  });
